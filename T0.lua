...
if game.PlaceId == 14433762945 then
    ...
    function GetRaids(Mode)
        if game:GetService("Workspace").Worlds:FindFirstChild("Hub") or game:GetService("Workspace").Worlds:FindFirstChild("Raids") then
            ...
            elseif Mode == "GetDailyChest" then
                for i,v in ipairs(game:GetService("Workspace").Worlds.Raids:GetDescendants()) do
                    if v.Name == "RaidChest2" and FindRaids(v:GetModelCFrame()) == FindRaids(game.Players.LocalPlayer.Character.HumanoidRootPart) then
                        if v:FindFirstChild('HumanoidRootPart') and v:FindFirstChild('HumanoidRootPart'):FindFirstChild('ChestPrompt') then
                            return v
                        end
                    end
    ...
    print(DateWorld[SaveSettings["Raids"]["Select Raids [World]"]].WorldName)
            ...
            elseif SaveSettings["Raids"]['Auto Farm Raid'] and game:GetService("Workspace").Worlds:FindFirstChild("Raids") then
                if LocalPlayer.PlayerGui.MainGui.HUD.RaidHUD.TimerDisplay.Timer:GetAttribute("EndTime") == 0 then
                    if SaveSettings["Raids"]['Collect Chest [After Finish]'] then
                        if SaveSettings["Raids"]['Collect Chest [After Finish]'] and not SaveSettings["Raids"]['Collect Chest [Daily Chest]'] and GetRaids("GetChest") == "None" then
                            local args = { [1] = "Hub" }
                            game:GetService("ReplicatedStorage").Remote.Player.Teleport:FireServer(unpack(args))
                            wait(.5)
                            repeat wait() until not LocalPlayer.PlayerGui:FindFirstChild('TeleportGui')
                        elseif SaveSettings["Raids"]['Collect Chest [After Finish]'] and SaveSettings["Raids"]['Collect Chest [Daily Chest]'] and GetRaids("GetChest") == "None" and GetRaids("GetDailyChest") == "None" then
                            local args = { [1] = "Hub" }
                            game:GetService("ReplicatedStorage").Remote.Player.Teleport:FireServer(unpack(args))
                            wait(.5)
                            repeat wait() until not LocalPlayer.PlayerGui:FindFirstChild('TeleportGui')
                        elseif SaveSettings["Raids"]['Collect Chest [After Finish]'] and not SaveSettings["Raids"]['Collect Chest [Daily Chest]'] and GetRaids("GetChest") ~= "None" and GetRaids("GetChest").HumanoidRootPart:FindFirstChild("ChestPrompt") then
                            ...
                            wait(5)
                        elseif SaveSettings["Raids"]['Collect Chest [After Finish]'] and SaveSettings["Raids"]['Collect Chest [Daily Chest]'] and GetRaids("GetDailyChest") ~= "None" and GetRaids("GetDailyChest").HumanoidRootPart:FindFirstChild("ChestPrompt") then
                            Character.HumanoidRootPart:PivotTo(GetRaids("GetChest"):GetModelCFrame())
                            wait(.1)
                            fireproximityprompt(GetRaids("GetChest").HumanoidRootPart.ChestPrompt)
                            wait(3)
