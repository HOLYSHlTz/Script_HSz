...
--item
function ItemWebhook()
    if Settings.ItemsWebhookEnabled then
	local url = Settings.ItemWebhookUrl
    print("webhook baby?")
    if url == "" then
        warn("Webhook Url is empty!")
        return
        ...
        for i,v in pairs(Table_All_Items_New_data) do
            if v['Count'] >= 0 and (v['Count']) >= 0 then
                if v['Count Shiny'] and v['Count'] then
                    if v['Count'] >= 0 or v['Count Shiny'] >= 0 then
                        if v['Count'] >= 0 and (v['Count']) >= 0 then
                        --if tostring(v['Name']) ~= "nil" or tostring(v['Name']) ~= "Portal" then
                            TextItemLabel = TextItemLabel .. tostring(CountAmount) .. ". " .. tostring(v['Name']) .. " : " .. tostring(v['Count']) .. "\n"
                            if v['Count Shiny'] >= 0 and (v['Count Shiny']) >= 0 then
                                --TextItemLabel = TextItemLabel .. " | " .. tostring(v['Name']) .. " (Shiny) : x" .. tostring(v['Count Shiny']) .. "\n"
                                CountAmount = CountAmount + 1
                            else
                                TextItemLabel = TextItemLabel .. "\n"
                                CountAmount = CountAmount + 1
                            end
                        end
                    end
                end
            elseif v['Count Shiny'] and v['Count Shiny'] > 0 and (v['Count Shiny']) > 0 then
                --TextItemLabel = TextItemLabel .. tostring(CountAmount) .. ". " .. tostring(v['Name']) .. " (Shiny) : " .. tostring(v['Count Shiny']) .. "\n"
                CountAmount = CountAmount + 1
            end
        end
            ...
            if v['Count'] >= 0 and (v['Count']) >= 0 then
                ...
                elseif string.find(i,"portal") or string.find(i,"disc") then
                    ...
                    if string.gsub(i, "%D", "") == "" then
                        ...
                        TextItemLabel = TextItemLabel .. tostring(CountAmount) .. ". " .. tostring(v['Name']) .. " Tier " .. tostring(string.gsub(i, "%D", "")) .. " : " .. tostring(v['Count']) .. "\n"
                    end
                    CountAmount = CountAmount + 1
                    else
                        TextItemLabel = TextItemLabel .. tostring(CountAmount) .. ". " .. tostring(v['Name']) .. " : " .. tostring(v['Count']) .. "\n"
        ...
        if TextItemLabel == "" then
            TextItemLabel = "Not Have Items Drops"
        end
		...
		local data = {
            ...
            warn("Sending items webhook notification...")
            request(sex)
        end
end
...
function Webhooksec()
    ...
    WebhookSec:Cheat("Textbox", "Item Webhook Url", function(Value)
        Settings.ItemWebhookUrl = Value
        saveSettings()
    end, {placeholder = Settings.ItemWebhookUrl})
    ...
    WebhookSec:Cheat("Checkbox","เปิดใช้ Items Webhook [All Items]", function(bool)
        Settings.ItemsWebhookEnabled = bool
        saveSettings()
    end,{enabled = Settings.ItemsWebhookEnabled})
...
function Webhooksec2()
    WebhookSec:Cheat("Button", "Test Item&Baby&Shop Webhook", function()
        print(Settings.WebhookUrl)
        ItemWebhook()
        BabyWebhook()
        SnipeShopNew()
