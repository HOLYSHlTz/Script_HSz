...
local function main()
    ...
    Explorer.ShowRightClick = function()
        ...
        if presentClasses["BasePart"] or presentClasses["Model"] then
            ...
            context:AddRegistered("COPY_CFRAME")
    ...
    Explorer.InitRightClick = function()
        ...
        context:Register("COPY_CFRAME",{Name = "Copy CFrame", IconMap = Explorer.MiscIcons, Icon = "TeleportTo", OnClick = function()
            local sList = selection.List
            local isa = game.IsA
            ...
            for i = 1,#sList do
                ...
                if isa(node.Obj,"BasePart") then
                    local PartPosCopy = node.Obj.CFrame + Settings.Explorer.TeleportToOffset
                    env.setclipboard(`CFrame.new({tostring(PartPosCopy)})`)
                    break
                elseif isa(node.Obj,"Model") then
                    if node.Obj.PrimaryPart then
                        local PartPosCopy = node.Obj.PrimaryPart.CFrame + Settings.Explorer.TeleportToOffset
                        env.setclipboard(`CFrame.new({tostring(PartPosCopy)})`)
                        break
                    else
                        local part = node.Obj:FindFirstChildWhichIsA("BasePart",true)
                        if part and nodes[part] then
                            local PartPosCopy = nodes[part].Obj.CFrame + Settings.Explorer.TeleportToOffset
                            env.setclipboard(`CFrame.new({tostring(PartPosCopy)})`)
                        end
                    end
                end
