...
-------------------
    ...
    SelectWorld:Cheat("Checkbox","🔎 Find Matchmaking", function(bool)
        print(bool)
...
local function startfarming()
    if game.PlaceId == 8304191830 and not Settings.farmprotal and Settings.autostart and Settings.AutoFarm and Settings.teleporting and not Settings.AutoInfinityCastle then
        ...
        elseif cata == "Raid Worlds" and Settings.Matchmaking then
            getgenv().door =  "_lobbytemplate212" or "_lobbytemplate213" or "_lobbytemplate214" or "_lobbytemplate215" or "_lobbytemplate216" 
            if tostring(game.Workspace._RAID.Raid[getgenv().door].Owner.Value) ~= plr.Name then
                for i, v in pairs(game:GetService("Workspace")["_RAID"].Raid:GetDescendants()) do
                    if v.Name == "Owner" and v.Value == nil then
                        ...
                        local args = {
                            [1] = Settings.SelectedLevel -- World/Level
                         }
...
--end]]
--Matckmaking_TEST
local function startmatckmaking()
    if game.PlaceId == 8304191830 and not Settings.farmprotal and Settings.autostart and Settings.AutoFarm and Settings.Matchmaking and Settings.teleporting and not Settings.AutoInfinityCastle then
        local cpos = plr.Character.HumanoidRootPart.CFrame; cata = Settings.WorldCategory; level = Settings.SelectedLevel;
        ...
        elseif cata == "Dungeon" then
            if level == "namek_halloween" then
                ...
                pcall(function() 
                    ...
                    print("send Webhook")
                    task.wait(0.5)
                    warn("Matck Find DUNGEONS Annivesary farming")
                    task.wait(1)
                end
            end
        end
    ...
    while task.wait() do
        if not Settings.AutoInfinityCastle then
            if not checkChallenge() then --Challenge_Not_Complete
                ...
                elseif not Settings.AutoChallenge and checkReward() == false then
                    startfarming()--S_Farming
                elseif not Settings.AutoChallenge and Settings.Matchmaking then
                    startmatckmaking()
                end
            elseif checkChallenge() == true then
                ...
                BabyWebhook()
                SnipeShopNew()
            elseif checkChallenge() == true and Settings.Matchmaking then
                startmatckmaking()--S_Farming
                BabyWebhook()
                SnipeShopNew()
            end
        elseif not Settings.AutoInfinityCastle == true then--Infiniy Castle
            if not checkChallenge() then --Challenge_Not_Complete
                if  Settings.AutoChallengeAll then
                    startChallenge() --S_Challenge
                elseif not Settings.AutoChallengeAll then
                    startfarming()--S_Farming
                elseif not Settings.AutoChallengeAll and Settings.Matchmaking then
                    startmatckmaking()
                end
            elseif checkChallenge() == true then
                startfarming()--S_Farming
