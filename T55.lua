                            ...
                            if v._stats.id.Value ~= "erwin" and v._stats.id.Value ~= "wendy" and v._stats.id.Value ~= "leafa" and v._stats.id.Value ~= "aot_generic" then
                                if v._stats.active_attack.Value ~= "nil" and tonumber(Settings.AutoSkillWave) <= S_wave.Value then
                                    if Settings.SelectedSkillUse2 == "BossDistance" then
                                        -- Check Infinite
                                        if GLD()._gamemode == "infinite" then
                                            if tostring(BossName) ~= "nil" and tostring(Settings.UnitDistanceX) >= tostring(string.format('%d', distanceBoss2)) or tostring(string.format('%d', distanceBoss2)) <= tostring(Settings.UnitDistanceX) then
                                    ...
                                    if tostring(v["_stats"].player.Value) == RYY3 then
                                        ...
                                        if v._stats.id.Value ~= "erwin" and v._stats.id.Value ~= "wendy" and v._stats.id.Value ~= "leafa" and v._stats.id.Value ~= "aot_generic" then
                                            if v._stats.active_attack.Value ~= "nil" and tonumber(Settings.AutoSkillWave) <= S_wave.Value then
                                                if Settings.SelectedSkillUse2 == "GBCDDistance" then 
                                                    if v._stats.state.Value == "formation" then
                                                    if tonumber(Settings.UnitDistanceX) >= tostring(string.format('%02d', distanceU3)) or tostring(string.format('%02d', distanceU3)) <= tonumber(Settings.UnitDistanceX) then
                                                        game:GetService("ReplicatedStorage").endpoints.client_to_server.use_active_attack:InvokeServer(v)
                                                        warn("Use Skill : " ..tostring(v._stats.id.Value) .. " | Method : "  ..tostring(NameSkill) .. " | Distance : "  ..string.format('%02d', distanceU3) .. " | Wave : " ..tostring(GetWaveNumber()))
                                                        end
                                                    end
                                                end
                                            end
                                    ...
                                    if tostring(v["_stats"].player.Value) == RYY3 then
                                        ...
                                        if v._stats.id.Value ~= "erwin" and v._stats.id.Value ~= "wendy" and v._stats.id.Value ~= "leafa" and v._stats.id.Value ~= "aot_generic" then
                                            if v._stats.active_attack.Value ~= "nil" and tonumber(Settings.AutoSkillWave) <= S_wave.Value then
                                                if Settings.SelectedSkillUse2 == "ATKDistance" then 
                                                    if v._stats.state.Value == "attack" then
                                                    if tonumber(Settings.UnitDistanceX) >= tostring(string.format('%02d', distanceU5)) or tostring(string.format('%02d', distanceU5)) <= tonumber(Settings.UnitDistanceX) then
                                                        wait(1.5)
                                                        game:GetService("ReplicatedStorage").endpoints.client_to_server.use_active_attack:InvokeServer(v)
                                                        warn("Use Skill : " ..tostring(v._stats.id.Value) .. " | Method : "  ..tostring(NameSkill) .. " | Distance : "  ..string.format('%02d', distanceU5) .. " | Wave : " ..tostring(GetWaveNumber()))
                                                        end
                                                    end
                                                end
                                            end
                                ...
                                for i, v in ipairs(game:GetService("Workspace")["_UNITS"]:GetChildren()) do
                                    if tostring(v["_stats"].player.Value) == RYY3 then
                                        ...
                                        if v._stats.id.Value ~= "erwin" and v._stats.id.Value ~= "wendy" and v._stats.id.Value ~= "leafa" and v._stats.id.Value ~= "aot_generic" then
                                            if v._stats.active_attack.Value ~= "nil" and tonumber(Settings.AutoSkillWave) <= S_wave.Value then
                                                if Settings.SelectedSkillUse2 == "distanceCount" then 
                                                    if tonumber(Settings.UnitDistanceX) >= tostring(string.format('%02d', distanceU2)) or tostring(string.format('%02d', distanceU2)) <= tonumber(Settings.UnitDistanceX) then
                                                        wait(1.5)
                                                        game:GetService("ReplicatedStorage").endpoints.client_to_server.use_active_attack:InvokeServer(v)
                                                        warn("Use Skill : " ..tostring(v._stats.id.Value) .. " | Method : "  ..tostring(NameSkill) .. " | Distance : "  ..string.format('%02d', distanceU2) .. " | Wave : " ..tostring(GetWaveNumber()))
                                                    end
                                                end
                                            end
                                        end
