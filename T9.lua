            ...
            elseif SaveSettings["Raids"]['Auto Farm Raid'] and game:GetService("Workspace").Worlds:FindFirstChild("Raids") then
                if LocalPlayer.PlayerGui.MainGui.HUD.RaidHUD.TimerDisplay.Timer:GetAttribute("EndTime") == 0 then
                    if SaveSettings["Raids"]['Collect Chest [After Finish]'] then
                        if not SaveSettings["Raids"]['Collect Chest [Daily Chest]'] and GetRaids("GetChest") == "None" then
                            local args = { [1] = "Hub" }
                            game:GetService("ReplicatedStorage").Remote.Player.Teleport:FireServer(unpack(args))
                            wait(.5)
                            repeat wait() until not LocalPlayer.PlayerGui:FindFirstChild('TeleportGui')
                        elseif SaveSettings["Raids"]['Collect Chest [Daily Chest]'] and GetRaids("GetChest") == "None" and GetRaids("GetDailyChest") == "None" then
                            local args = { [1] = "Hub" }
                            game:GetService("ReplicatedStorage").Remote.Player.Teleport:FireServer(unpack(args))
                            wait(.5)
                            repeat wait() until not LocalPlayer.PlayerGui:FindFirstChild('TeleportGui')
                        elseif not SaveSettings["Raids"]['Collect Chest [Daily Chest]'] and GetRaids("GetChest") ~= "None" and GetRaids("GetChest").HumanoidRootPart:FindFirstChild("ChestPrompt") then
                            ...
                            wait(5)
                        elseif SaveSettings["Raids"]['Collect Chest [Daily Chest]'] and GetRaids("GetDailyChest") ~= "None" and GetRaids("GetDailyChest").HumanoidRootPart:FindFirstChild("ChestPrompt") then
