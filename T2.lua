...
LowCPU2:Cheat("Checkbox","Mobile Low CPU Mode ", function(bool)
	print(bool)
...
task.spawn(function()
	while task.wait() do
        if isrbxactive3() ~= true and Settings.lowCpuModeM then
            setfpscap(30)
            game:GetService("RunService"):Set3dRenderingEnabled(false)
        else
            setfpscap(1000)
...
UserInputService.InputBegan:Connect(function(R)
    if R.KeyCode == Enum.KeyCode.G then
        if isrbxactive4() ~= true then
            setfpscap(tonumber(Settings.FPSCAPNum))
            game:GetService("RunService"):Set3dRenderingEnabled(true)
            isrbxactive4(true)
        else
