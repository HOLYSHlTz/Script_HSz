...
if game.PlaceId == 14433762945 then
    ...
    for i,v in pairs(WorldDate) do
        if v.WorldNum > -1 and i ~= "Test" then
            ...
            local Enemie = require(ReplicatedStorage.ModuleScripts.Config.EnemyStats:FindFirstChild(DateWorld[SaveSettings["Raids"]["Select Raids [World]"]].WorldName)) 
            or require(ReplicatedStorage.ModuleScripts.Config.EnemyStats:FindFirstChild(DateWorld[SaveSettings["Auto Farm"]["Select World"]].WorldName))
            or require(ReplicatedStorage.ModuleScripts.Config.EnemyStats:FindFirstChild(i))
